...
% Choose default command line output for OFFSET_ERR_AND_NOISE_MASKING
    ...
    % initial IPCMRA JBOCK
    %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
    [~,~,~,d] = size(handles.MR_FFE_FH);
    IPCMRA = sqrt((1/d)*sum( ((handles.MR_FFE_FH).^2).*((handles.MR_PCA_FH).^2 + (handles.MR_PCA_AP).^2 + (handles.MR_PCA_RL).^2),4));
    ...
    if handles.id ==1
        ...
        handles.list_string2 = {'Select the property to modify ...','Static Tissue','Noise','ROI','IPCMRA J.BOCK 2009', 'IPCMRA M.MARKL 2007', 'IPCMRA A.HENNEMUTH 2011', 'IPCMRA J.BOCK 2009 + DIV-MED', 'IPCMRA M.MARKL 2007 + DIV-MED', 'IPCMRA A.HENNEMUTH 2011 + DIV-MED'};
...
% handles    structure with handles and user data (see GUIDATA)
    ...
    elseif handles.id_variable==6 % michael markl  
        ...
        %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
        [~,~,~,d] = size(handles.MR_FFE_FH);
        IPCMRA_IND = ((handles.MR_FFE_FH)).*sqrt(((handles.MR_PCA_FH).^2 + (handles.MR_PCA_AP).^2 + (handles.MR_PCA_RL).^2));
        IPCMRA = (1/d)*sum(IPCMRA_IND.^2,4);
        handles.IPCMRA = (IPCMRA/max(IPCMRA(:)))*255;
    ...
    elseif handles.id_variable==7 % A Hennemuth  
        ...
        %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
        [~,~,~,d] = size(handles.MR_FFE_FH);
        IPCMRA_IND = ((handles.MR_FFE_FH).^2).*(((handles.MR_PCA_FH).^2 + (handles.MR_PCA_AP).^2 + (handles.MR_PCA_RL).^2));
        IPCMRA = sqrt((1/d)*sum(IPCMRA_IND,4));
        handles.IPCMRA = (IPCMRA/max(IPCMRA(:)))*255;
    ...
    elseif handles.id_variable==8 % JBOCK - JSOTELO
        ...
        f = waitbar(.33,'Please wait fixing IPCMRA using the divergence...');
        
        [~,~,~,d] = size(handles.MR_FFE_FH);
        % calculamos la divergencia.
        div = zeros(size(handles.MR_PCA_FH));
        [X,Y,Z] = meshgrid(1:size(handles.MR_PCA_FH,2),1:size(handles.MR_PCA_FH,1),1:size(handles.MR_PCA_FH,3));
        for n = 1:size(handles.MR_FFE_FH,4)
            div(:,:,:,n) = divergence(X,Y,Z,handles.MR_PCA_FH(:,:,:,n),handles.MR_PCA_AP(:,:,:,n),handles.MR_PCA_RL(:,:,:,n));
        end

        diver = std(abs(div),0,4);
        div = diver/max(diver(:));
        
        waitbar(.66,f,'Please wait fixing IPCMRA using the divergence...');
        % aplicamos filtros de mediana
        B = medfilt3(log(div),[5,5,5],'symmetric');
        B(isinf(B)) = NaN;
        B = B + abs(min(B(:)));
        B(isnan(B)) = 0;
        B= B/max(B(:));

        % calculamos la ipcmra enhanced
        IPCMRA = sqrt((1/d)*sum( ((handles.MR_FFE_FH).^2).*((handles.MR_PCA_FH.*(1-B)).^2 + (handles.MR_PCA_AP.*(1-B)).^2 + (handles.MR_PCA_RL.*(1-B)).^2),4));
        handles.IPCMRA = medfilt3(IPCMRA,[3,3,3]);
        handles.IPCMRA = medfilt3(handles.IPCMRA,[3,3,3]);
        handles.IPCMRA = (handles.IPCMRA/max(handles.IPCMRA(:)))*255;
        waitbar(1,f,'Please wait fixing IPCMRA using the divergence...');
        pause(0.25)
    ...
    elseif handles.id_variable==9 % michael markl - JSOTELO
        ...
        % calculamos la ipcmra enhanced
        IPCMRA_IND = ((handles.MR_FFE_FH)).*sqrt(((handles.MR_PCA_FH.*(1-B)).^2 + (handles.MR_PCA_AP.*(1-B)).^2 + (handles.MR_PCA_RL.*(1-B)).^2));
        IPCMRA = (1/d)*sum(IPCMRA_IND.^2,4);
        IPCMRA = medfilt3(IPCMRA,[3,3,3]);
        IPCMRA = medfilt3(IPCMRA,[3,3,3]);
        handles.IPCMRA = (IPCMRA/max(IPCMRA(:)))*255;
    ...
    elseif handles.id_variable==10 % A Hennemuth - JSOTELO
        ...
        %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
        [~,~,~,d] = size(handles.MR_FFE_FH);
        IPCMRA_IND = ((handles.MR_FFE_FH).^2).*(((handles.MR_PCA_FH.*(1-B)).^2 + (handles.MR_PCA_AP.*(1-B)).^2 + (handles.MR_PCA_RL.*(1-B)).^2));
        IPCMRA = sqrt((1/d)*sum(IPCMRA_IND,4));
        IPCMRA = medfilt3(IPCMRA,[3,3,3]);
        IPCMRA = medfilt3(IPCMRA,[3,3,3]);
        handles.IPCMRA = (IPCMRA/max(IPCMRA(:)))*255;
...
% handles    structure with handles and user data (see GUIDATA)
    ...
    elseif handles.id_variable ==3
        ...
        close(h)
        handles.IPCMRA = abs(repmat(noise,1,1,1)-1).*handles.IPCMRA;
    ...
    elseif handles.id_variable ==4
        ...
        close(h)
        handles.IPCMRA       = mask_roi(:,:,:,1).*handles.IPCMRA;
